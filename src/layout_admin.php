...
if (!isset($pageTitle))  $pageTitle  = 'openGPLMS';
...
<title><?= h($pageTitle) ?> — Σύστημα Διαχείρισης Βιβλιοθήκης — openGPLMS v1.0</title>
...
<div id="sidebar">
    <div class="brand">
        <div class="brand-title">openGPLMS</div>
        <div class="brand-sub">General Purpose Library Management System</div>
        <div class="brand-sub">Open Source Library Management System</div>
        <div class="brand-sub">Digital Library</div>
    </div>
    ...
    <nav>
        ...
        <a href="messages.php" class="<?= $activePage==='messages'?'active':'' ?>" style="position:relative">
            ...
            <?php
            $db = getDB();
            $unreadNav = $db->prepare("SELECT COUNT(*) FROM " . TABLE_PREFIX . "messages WHERE to_user=? AND is_read=0");
