...
try {
    ...
    $employeeHash = password_hash('gplms123', PASSWORD_DEFAULT);

    $stmt = $pdo->prepare("
    INSERT INTO `" . TABLE_PREFIX . "users` 
        (`id`, `username`, `password`, `role`, `active`, `created_at`) 
    VALUES
        (1, 'gplmsadm',    ?, 'admin',    1, '2026-02-18 11:15:29'),
        (2, 'gplmslib', ?, 'employee', 1, '2026-02-18 11:15:29')
	");
	$stmt->execute([$adminHash, $employeeHash]);
    ok("Default users inserted &nbsp;<code>gplmsadm / admin123</code> &nbsp;&amp;&nbsp; <code>gplmslib / gplms123</code>");
