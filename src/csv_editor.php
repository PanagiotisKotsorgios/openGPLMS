...
$categories = $db->query("SELECT name FROM " . TABLE_PREFIX . "categories ORDER BY name")->fetchAll(PDO::FETCH_COLUMN);
$publishers  = $db->query("SELECT name FROM " . TABLE_PREFIX . "publishers ORDER BY name")->fetchAll(PDO::FETCH_COLUMN);
...
function downloadCSV() {
    ...
    a.download = 'opengplms_library_' + new Date().toISOString().slice(0,10) + '.csv';
    a.click();
}
