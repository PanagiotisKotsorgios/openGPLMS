...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if (!$uname || !$pass) {
            ...
            $chk = $db->prepare("SELECT id FROM " . TABLE_PREFIX . "users WHERE username=?");
            $chk->execute([$uname]);
            if ($chk->fetch()) {
                flash('Το username υπάρχει ήδη.', 'error');
            } else {
                $db->prepare("INSERT INTO " . TABLE_PREFIX . "users (username,password,role,active) VALUES (?,?,?,1)")
        ...
        if ($uid === (int)$_SESSION['user_id']) {
            ...
            $db->prepare("UPDATE " . TABLE_PREFIX . "users SET active=1-active WHERE id=?")->execute([$uid]);
        ...
        if ($uid === (int)$_SESSION['user_id']) {
            ...
            $db->prepare("UPDATE " . TABLE_PREFIX . "users SET role=? WHERE id=?")->execute([$role, $uid]);
        ...
        if ($uid === (int)$_SESSION['user_id']) {
            ...
            $uRow = $db->prepare("SELECT username FROM " . TABLE_PREFIX . "users WHERE id=?");
            $uRow->execute([$uid]);
            $uName = $uRow->fetchColumn();
            $db->prepare("DELETE FROM " . TABLE_PREFIX . "users WHERE id=?")->execute([$uid]);
            $db->prepare("INSERT INTO " . TABLE_PREFIX . "audit_log (user_id,action,target_type,details) VALUES (?,?,?,?)")
 ...
 */
$q = "SELECT u.*,
      (SELECT COUNT(*) FROM " . TABLE_PREFIX . "books       WHERE created_by=u.id)                  AS book_count,
      (SELECT COUNT(*) FROM " . TABLE_PREFIX . "messages    WHERE from_user=u.id OR to_user=u.id)   AS msg_count,
      (SELECT MAX(created_at) FROM " . TABLE_PREFIX . "audit_log WHERE user_id=u.id)                AS last_activity
      FROM " . TABLE_PREFIX . "users u WHERE 1=1";
$p = [];
...
$activeUsers   = $db->query("SELECT COUNT(*) FROM " . TABLE_PREFIX . "users WHERE active=1")->fetchColumn();
$adminCount    = $db->query("SELECT COUNT(*) FROM " . TABLE_PREFIX . "users WHERE role='admin'")->fetchColumn();
$employeeCount = $db->query("SELECT COUNT(*) FROM " . TABLE_PREFIX . "users WHERE role='employee'")->fetchColumn();
