...
try {
    ...
    (1, 'admin',    '$adminHash',    'admin',    1, '2026-02-18 11:15:29'),
    (2, 'employee', '$employeeHash', 'employee', 1, '2026-02-18 11:15:29')
    ");
    ok("Default users inserted &nbsp;<code>admin / admin123</code> &nbsp;&amp;&nbsp; <code>employee / employee123</code>");
    ...
    ════════════════════════════════════════════════════════ */
    $pdo->exec("DROP TABLE IF EXISTS `publishers`");
    $pdo->exec("
    ...
    $stmt = $pdo->prepare("INSERT INTO `books` $cols VALUES $ph,$ph,$ph,$ph,$ph");
    $stmt->execute([
        // 1. Βιβλίο — Ιστορία
    ...
    <table style='font-family:sans-serif;font-size:14px;border-collapse:collapse;margin-bottom:16px'>
        ...
        <tbody>
            <tr>
                ...
                <td style='padding:6px 20px 6px 0'>admin123</td>
            ...
            <tr>
                ...
                <td style='padding:6px 20px 6px 0'>employee123</td>
    ...
    /*
     * Αν αποτύχει οποιοδήποτε βήμα, το PDO ρίχνει exception λόγω ERRMODE_EXCEPTION.
     * Επαναφορά FK_CHECKS για να μην μείνει απενεργοποιημένο στη σύνδεση.
     */
