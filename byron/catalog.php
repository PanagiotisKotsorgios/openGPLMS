...
function getAdminIds(PDO $db): array {
    $ids = $db->query("SELECT id FROM users WHERE role='admin' AND active=1 ORDER BY id ASC")->fetchAll(PDO::FETCH_COLUMN);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete_book' && isset($_POST['delete_id'])) {
        ...
        $stmt = $db->prepare("SELECT id, title, isbn, created_by FROM books WHERE id=?");
        $stmt->execute([$delId]);
        $book = $stmt->fetch();
        ...
        $db->prepare("DELETE FROM books WHERE id=?")->execute([$delId]);
    ...
    if ($action === 'mass_delete' && isAdmin()) {
        ...
        $deleted = 0;
        foreach ($ids as $did) {
            $res = $db->prepare("DELETE FROM books WHERE id=?");
            $res->execute([$did]);
            if ($res->rowCount() > 0) {
                $db->prepare("INSERT INTO audit_log (user_id, action, target_type, target_id, details) VALUES (?,?,?,?,?)")
    ...
    if ($action === 'mass_status' && isAdmin()) {
        ...
        $updated = 0;
        foreach ($ids as $uid) {
            $res = $db->prepare("UPDATE books SET status=? WHERE id=?");
    ...
    if ($action === 'mass_visibility' && isAdmin()) {
        ...
        $updated = 0;
        foreach ($ids as $uid) {
            $res = $db->prepare("UPDATE books SET is_public=? WHERE id=?");
            $res->execute([$vis, $uid]);
    ...
    if ($action === 'request_permission' && isset($_POST['book_id'], $_POST['req_action'])) {
        ...
        $stmt = $db->prepare("SELECT id, title, author, isbn, created_by FROM books WHERE id=?");
        $stmt->execute([$bookId]);
        $book = $stmt->fetch();
        ...
        $ins = $db->prepare("INSERT INTO messages (from_user, to_user, subject, body) VALUES (?,?,?,?)");
...
$totalStmt = $db->prepare("SELECT COUNT(*) FROM books b WHERE $whereStr");
...
$languages  = $db->query("SELECT DISTINCT language FROM books WHERE language IS NOT NULL ORDER BY language")->fetchAll(PDO::FETCH_COLUMN);
