...
$newMonth     = $db->query("SELECT COUNT(*) FROM books WHERE MONTH(created_at)=MONTH(NOW()) AND YEAR(created_at)=YEAR(NOW())")->fetchColumn();
...
$typeStats    = $db->query("SELECT type, COUNT(*) as cnt FROM books GROUP BY type ORDER BY cnt DESC")->fetchAll();
$catStats     = $db->query("SELECT c.name, COUNT(b.id) as cnt FROM categories c LEFT JOIN books b ON b.category_id=c.id GROUP BY c.id, c.name ORDER BY cnt DESC")->fetchAll();
$langStats    = $db->query("SELECT language, COUNT(*) as cnt FROM books GROUP BY language ORDER BY cnt DESC LIMIT 8")->fetchAll(); // Top 8 για ευανάγνωστο chart
$yearStats    = $db->query("SELECT year, COUNT(*) as cnt FROM books WHERE year IS NOT NULL GROUP BY year ORDER BY year DESC LIMIT 10")->fetchAll();
$monthlyStats = $db->query("SELECT DATE_FORMAT(created_at,'%Y-%m') as mo, COUNT(*) as cnt FROM books GROUP BY mo ORDER BY mo DESC LIMIT 12")->fetchAll(); // 12 μήνες πίσω
...
<!-- AUDIT LOG -->
<div class="card-panel">
    <h6 style="font-family:'Playfair Display',serif;font-size:15px;margin-bottom:16px">Ιστορικό Ενεργειών</h6>
