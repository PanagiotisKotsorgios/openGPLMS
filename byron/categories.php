...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_cat') {
        ...
        if ($name) {
            try {
                $db->prepare("INSERT INTO publishers (name) VALUES (?)")->execute([$name]);
...
$stmt = $db->prepare("SELECT COUNT(*) FROM categories c WHERE $cWhere");
...
$stmt = $db->prepare("SELECT COUNT(*) FROM publishers p WHERE $pWhere");
