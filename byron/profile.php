...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!password_verify($curpass, $user['password'])) {
        ...
        $db->prepare("INSERT INTO audit_log (user_id, action, target_type) VALUES (?,?,?)")
...
$myUser = $db->prepare("SELECT * FROM users WHERE id=?");
$myUser->execute([$_SESSION['user_id']]);
$myUser = $myUser->fetch();

// Τελευταίες 10 ενέργειες του χρήστη — περιορισμός σε LIMIT 10 για απόδοση
$myActions = $db->prepare("SELECT * FROM audit_log WHERE user_id=? ORDER BY created_at DESC LIMIT 10");
