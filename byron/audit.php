...
/* ─────────────────────────────────────────────────────────────
   ...
   URL ενεργοποίησης: audit.php?backup=1
...
$totalStmt = $db->prepare("SELECT COUNT(*) FROM audit_log a LEFT JOIN users u ON a.user_id=u.id WHERE $whereStr");
...
$stats = $db->query("SELECT action, COUNT(*) as cnt FROM audit_log GROUP BY action ORDER BY cnt DESC")->fetchAll();
$totalEvents = (int)$db->query("SELECT COUNT(*) FROM audit_log")->fetchColumn();
...
<button type="button" class="btn btn-outline-gold" onclick="openBackupModal()">
    <i class="bi bi-shield-check me-1"></i> Δημιουργία Αντιγράφου Ασφαλείας
        ...
        <a href="?<?= qsKeep(['export'=>1]) ?>" class="btn btn-gold">
