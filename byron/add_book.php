...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$title || !$author) {
        ...
        if ($isEdit) {
            ...
            $db->prepare("UPDATE books SET title=?,author=?,isbn=?,type=?,category_id=?,publisher_id=?,year=?,language=?,pages=?,edition=?,volume=?,location=?,description=?,status=?,is_public=?,cover_url=? WHERE id=?")
            ...
            $stmt = $db->prepare("INSERT INTO books (title,author,isbn,type,category_id,publisher_id,year,language,pages,edition,volume,location,description,status,is_public,cover_url,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
