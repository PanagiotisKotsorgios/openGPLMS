...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'send') {
        ...
        if ($to && $body) {
            ...
            $chk = $db->prepare("SELECT id FROM " . TABLE_PREFIX . "users WHERE id=? AND active=1");
            $chk->execute([$to]);
            if ($chk->fetch()) {
        ...
        $db->prepare("DELETE FROM " . TABLE_PREFIX . "messages WHERE id=? AND (to_user=? OR from_user=?)")
           ->execute([$mid, $_SESSION['user_id'], $_SESSION['user_id']]);
        flash('Το μήνυμα διαγράφηκε.');

    } elseif ($action === 'mark_all_read') {
        $db->prepare("UPDATE " . TABLE_PREFIX . "messages SET is_read=1 WHERE to_user=? AND is_read=0")
...
if ($viewId && $tab === 'inbox') {
    $db->prepare("UPDATE " . TABLE_PREFIX . "messages SET is_read=1 WHERE id=? AND to_user=?")
...
if ($tab === 'sent') {
    $listQ = "SELECT m.*, u.username AS other_name, u.role AS other_role
              FROM " . TABLE_PREFIX . "messages m LEFT JOIN " . TABLE_PREFIX . "users u ON u.id = m.to_user
              WHERE m.from_user = ?";
    $listP = [$_SESSION['user_id']];
} else {
    $listQ = "SELECT m.*, u.username AS other_name, u.role AS other_role
              FROM " . TABLE_PREFIX . "messages m LEFT JOIN " . TABLE_PREFIX . "users u ON u.id = m.from_user
              WHERE m.to_user = ?";
    $listP = [$_SESSION['user_id']];
}
if ($search) {
...
$unreadStmt = $db->prepare("SELECT COUNT(*) FROM " . TABLE_PREFIX . "messages WHERE to_user=? AND is_read=0");
...
if ($viewId) {
    $stmt = $db->prepare("
        ...
        LEFT JOIN " . TABLE_PREFIX . "users fu ON fu.id = m.from_user
        LEFT JOIN " . TABLE_PREFIX . "users tu ON tu.id = m.to_user
 ...
 */
$usersStmt = $db->query("SELECT id, username, role FROM " . TABLE_PREFIX . "users
