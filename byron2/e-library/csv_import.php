...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csv_file'])) {
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
        ...
        while (($row = fgetcsv($handle, 2000, $delimiter)) !== false) {
            ...
            $catId = $catName ? findOrCreate($db, TABLE_PREFIX . 'categories', $catName) : null;
            $pubId = $pubName ? findOrCreate($db, TABLE_PREFIX . 'publishers', $pubName) : null;

            // Έλεγχος αν το ISBN υπάρχει ήδη στη βάση
            $existingId = null;
            if ($isbn) {
                $chk = $db->prepare("SELECT id FROM " . TABLE_PREFIX . "books WHERE isbn=?");
                $chk->execute([$isbn]);
                $ex = $chk->fetch();
            ...
            if ($existingId && $updateExisting) {
                ...
                $db->prepare("UPDATE " . TABLE_PREFIX . "books SET title=?,author=?,type=?,category_id=?,publisher_id=?,year=?,language=?,pages=?,edition=?,volume=?,location=?,description=?,status=?,is_public=?,cover_url=? WHERE id=?")
                ...
                $db->prepare("INSERT INTO " . TABLE_PREFIX . "books (title,author,isbn,type,category_id,publisher_id,year,language,pages,edition,volume,location,description,status,is_public,cover_url,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
